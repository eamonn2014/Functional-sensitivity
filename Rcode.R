...
# Create data, range of independent variable
  ...
  loq1 <- function (x, y, model, spec, print.plot=1, Xspec)  {
    ...
    p <- p +  theme(panel.background=element_blank(),
                    ...
                    axis.title.x = element_text(size = rel(1.1), angle = 00),
                    axis.title = element_text(size = 16, angle = 00),
                   # panel.grid.minor.x = element_line( size=0.5 ,  linetype = 'solid', colour = "gray88"),
  ...
  # run through all models and select best model
   ...
   #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    model <- which(ssr==min(ssr)) 
    mdata <- res$foo
    res2 <- loq1(x=x, y=y, model=model, spec=  NA, print.plot=1,  Xspec=NA)  # run best model
    
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    
    res2 <- loq1(x=x, y=y, model=3, spec=  NA, print.plot=1,  Xspec=NA)  # run best model
